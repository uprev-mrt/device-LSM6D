...
{
	lsm6d_write_reg(LSM6D_REG_CTRL1_XL, rate);
}

void lsm6d_set_gyro_odr(uint8_t rate)
{
	lsm6d_write_reg(LSM6D_REG_CTRL2_G, rate);
}

void lsm6d_get_fifo(accel_vector_t* result)
{
	int16_t raw_x, raw_y, raw_z;
	lsm6d_read_regs(LSM6D_REG_FIFO_DATA_L, (uint8_t*) &raw_x, 2);
	lsm6d_read_regs(LSM6D_REG_FIFO_DATA_L, (uint8_t*) &raw_y, 2);
	lsm6d_read_regs(LSM6D_REG_FIFO_DATA_L, (uint8_t*) &raw_z, 2);
...
void lsm6d_poll(accel_vector_t* result)
{
  ...
  lsm6d_read_regs(LSM6D_REG_OUT_X, (uint8_t*) data, 6);
...
{
	HAL_StatusTypeDef result = HAL_I2C_Mem_Write(I2cHandle, LSM6D_8BIT_ADDR, (uint16_t) reg , I2C_MEMADD_SIZE_8BIT, data , (uint16_t) len, 2000);
	HAL_Delay(5);
	return result;
}
...
{
	HAL_StatusTypeDef result = HAL_I2C_Mem_Read(I2cHandle, LSM6D_8BIT_ADDR, (uint16_t) reg  , I2C_MEMADD_SIZE_8BIT, data, (uint16_t) len, 2000);
	HAL_Delay(5);
	return result;
}
